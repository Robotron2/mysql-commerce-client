	...
	const fetchProducts = async () => {
		...
		if (localProducts && page == 1) {
			...
			try {
				...
				if (page === 1) {
					sessionStorage.setItem("products-user", JSON.stringify(response.data.products))
	...
	return (
		<motion.div animate={{ opacity: 1 }} initial={{ opacity: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.8 }}>
			...
			<div className="md:px-14 lg:px-44 mb-96">
				...
				{categories.length > 0 && (
					<>
						<ProductFilter filter={filter} categories={categories} onChange={handleFilterChange} onApplyFilter={applyFilter} />
					</>
				)}
